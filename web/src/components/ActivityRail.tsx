...
export function activityEventCopy(event: InquiryActivityEvent, snapshot: InquiryTaskSnapshot): { title: string; detail: string } {
  ...
  const copy: Record<InquiryActivityEvent["type"], { title: string; detail: string }> = {
    ...
    credit_reserved: { title: "Reserved the spending limit", detail: "CallBridge reserved platform credits for this single attempt." },
    attempt_queued: { title: "Queued one call attempt", detail: "Automatic retry remains disabled." },
    dispatch_uncertain: { title: "Call creation needs verification", detail: "The provider outcome is uncertain. CallBridge will not dial again." },
    dispatch_failed: { title: "Call was not created", detail: "The provider confirmed that no call exists, and no retry was scheduled." },
    dispatch_reconciled: { title: "Verified the provider outcome", detail: "CallBridge reconciled the uncertain request before changing call state." },
    dialing: { title: "Dialing", detail: `CallBridge is dialing ${destination}.` },
    connected: { title: "Call connected", detail: "The destination answered and the approved disclosure is due first." },
    disclosure_delivered: { title: "AI disclosure delivered", detail: "The disclosure completed before CallBridge continued the inquiry." },
    ...
    end_requested: { title: "End requested", detail: "CallBridge asked the worker to end the current attempt." },
