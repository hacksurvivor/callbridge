...
import { CheckIcon, Loader2Icon, PhoneCallIcon, XIcon } from "lucide-react";
...
export function ApprovalCard({
  ...
  title: string;
  subtitle: string;
  onAllowOnce?: ComponentProps<"button">["onClick"];
  onAlwaysAllow?: ComponentProps<"button">["onClick"];
  onDeny?: ComponentProps<"button">["onClick"];
  allowOnceLabel?: string;
  alwaysAllowLabel?: string;
  denyLabel?: string;
}) {
  return (
    <div data-slot="approval-card" className={cn(paper, "flex w-full max-w-sm flex-col gap-3.5 rounded-[20px] p-4", className)} {...props}>
      <div className="flex items-center gap-3">
        <span className="bg-foreground/[0.05] text-foreground/45 flex size-9 shrink-0 items-center justify-center rounded-xl"><PhoneCallIcon className="size-4" /></span>
        <div className="flex flex-col"><p className="text-[13.5px] font-medium">{title}</p><p className="text-foreground/45 text-xs">{subtitle}</p></div>
      </div>
      <div className={cn(field, "text-foreground/70 rounded-xl px-3.5 py-2.5 font-mono text-xs")}>{command}</div>
